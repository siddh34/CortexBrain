...
fn init_veth_tracer(bpf: &mut Ebpf) -> Result<(), anyhow::Error> {
    ...
    //TODO: here i still need to exclude the kprobes that are not called during the deletion of a net device
    veth_deletion_tracer
        .attach("unregister_netdev", 0)
        .context("Failed to attach to unregister_netdev")?;
    veth_deletion_tracer
        .attach("unregister_netdevice_many", 0)
        .context("Failed to attach to unregister_netdevice_many")?;
    veth_deletion_tracer
        .attach("unregister_netdevice_many_notify", 0)
        .context("Failed to attach to unregister_netdevice_many_notify")?;
    veth_deletion_tracer
        .attach("unregister_netdevice_notifier", 0)
        .context("Failed to attach to unregister_netdevice_notifier")?;
    veth_deletion_tracer
        .attach("unregister_netdevice_notifier_dev_net", 0)
        .context("Failed to attach to unregister_netdevice_notifier_dev_net")?;
    veth_deletion_tracer
        .attach("unregister_netdevice_notifier_net", 0)
        .context("Failed to attach to unregister_netdevice_notifier_net")?;
    veth_deletion_tracer
...
async fn event_listener(bpf_maps: (Map, Map)) -> Result<(), anyhow::Error> {
    ...
    let net_events_running = veth_running.clone();

    //waiting for signint (CTRL+C) to stop the main program
    //FIXME: this async task causes problems with the net_events_running that is also called by the display_events function
    /*     tokio::spawn(async move {
        signal::ctrl_c().await.unwrap();
        net_events_running.store(false, Ordering::SeqCst);
    }); */
    ...
    display_events(perf_net_events_buffer, net_events_running, events_buffers);
    info!("Exiting...");
    Ok(())
}
